...
class Ppo:
    ...
    def train(self, memory):
        ...
        states = torch.tensor(np.array(list(memory[:, 0])), dtype=torch.float32)
        actions = torch.tensor(np.array(list(memory[:, 1])), dtype=torch.float32)
        rewards = torch.tensor(np.array(list(memory[:, 2])), dtype=torch.float32)
        masks = torch.tensor(np.array(list(memory[:, 3])), dtype=torch.float32)
    ...
    def kl_divergence(self, old_mu, old_sigma, mu, sigma):
        ...
        kl = torch.log(old_sigma) - torch.log(sigma) + \
